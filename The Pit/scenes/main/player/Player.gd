...
const GRAVITY = 50
const JUMPFORCE = -750
const SHOOTFORCE = -1000  #SET TO 900 WHEN DONE TESTING
const HITBACK = -200
...
func get_input():
	...
	if velocity.length() <= 299:
		state_machine.travel("Idle")
	if velocity.length() > 299 and is_on_floor():
		state_machine.travel("Run")
	elif velocity.length() > 299 and is_on_floor() == false and Input.is_action_pressed("down"):
		state_machine.travel("LookingDown(OnlyInAir)")
		Global.scale_x = 0
	elif velocity.length() > 299 and is_on_floor() == false and Input.is_action_pressed("up"):
		state_machine.travel("FallLookingUp")
		Global.scale_x = 0
	elif velocity.length() > 299 and is_on_floor() == false:
...
func _on_DamageDetector_area_shape_entered(area_rid, area, area_shape_index, local_shape_index):
	pass
